...
func (manager *SysProcessManager) Init() {
	debugJson("Init")
	manager._log = make(chan Process, 100)
}

func (manager *SysProcessManager) UpdatePorts() {
	debugJson("UpdatePorts")
	manager._tcp = Tcp()
	manager._tcp6 = Tcp6()
	manager._udp = Udp()
	manager._udp6 = Udp6()
	emitLine(logLevel.verbose, "TCP %d TCP6 %d UDP  %d UDP6  %d", len(manager._tcp), len(manager._tcp6), len(manager._udp), len(manager._udp6))
}
...
func (manager *SysProcessManager) GetProcessInfoByLocalPort(localPort uint32, localIpAddress string) *Process {
	...
	for _, elem := range manager._tcp {
		...
		currentPort := uint32(elem.Port)
		if currentIp == localIpAddress && currentPort == localPort {
			founded = elem
			find = true
		}
	}

	for _, elem := range manager._tcp6 {
		currentIp := string(elem.Ip)
		currentPort := uint32(elem.Port)
		if currentIp == localIpAddress && currentPort == localPort {
			founded = elem
	...
	for _, elem := range manager._udp6 {
		currentIp := string(elem.Ip)
		currentPort := uint32(elem.Port)
		if currentIp == localIpAddress && currentPort == localPort {
			founded = elem
